...
(function (factory) {
    ...
    $.fn.tweet = function (o) {
        ...
        function load(widget) {
            ...
            if (s.loading_text) {
                $(widget).not(":has(.slides)").empty().append(loading);
            }
...
jQuery(document).ready(function () {
    if (jQuery().tweet) {
	    jQuery('.tweets_display').each(function(){
				var $tw1 = jQuery(this);
		    var limit = $tw1.attr("data-limit");
		    jQuery($tw1).tweet({
          template: "{time} {user} {text}",
          twitter_api_url: 'assets/twitter/proxy/twitter_proxy.php?limit='+limit
